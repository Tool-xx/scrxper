...
// Язык отчёта: opts.lang ('ru' | 'en'), по умолчанию 'ru'.
const REPORT_I18N = {
  ru: {
    profileReport: 'отчёт по профилю',
    verifiedOnly: 'Только верифицированные',
    verifiedTotal: 'верифицированных всего',
    following: 'Подписки',
    followers: 'Подписчики',
    verifiedWord: 'верифицированных',
    noVerifiedTitle: 'В этом списке нет верифицированных аккаунтов',
    noVerifiedSub: 'Собираются только профили со значком верификации.',
    thUser: 'Пользователь',
    thFollowers: 'Подписчики',
    thBio: 'Био',
    searchPh: 'Фильтр по нику или имени…',
    copy: '📋 Копировать',
    copyTitle: 'Скопировать все ники (@…)',
    copied: '✅ Скопировано',
    noMatches: 'Нет совпадений',
    noMatchesSub: 'Ничего не подходит под фильтр — попробуйте другой запрос.',
    footer1: 'Сгенерировано ScrXper v{v} · {date}',
    footer2: 'Только верифицированные аккаунты · число подписчиков и год создания собраны на x.com'
  },
  en: {
    ...
    noVerifiedTitle: 'No verified accounts in this list',
    noVerifiedSub: 'Only profiles with the verification badge are collected.',
    thUser: 'User',
    thFollowers: 'Followers',
    thBio: 'Bio',
    searchPh: 'Filter by username or name…',
    copy: '📋 Copy',
    copyTitle: 'Copy all usernames (@…)',
    copied: '✅ Copied',
    noMatches: 'No matches',
    noMatchesSub: 'Nothing matches your filter — try another query.',
    footer1: 'Generated by ScrXper v{v} · {date}',
    footer2: 'Verified accounts only · follower counts and account age collected on x.com'
  }
};
...
function sectionHtml(secId, title, users, results, L) {
  if (!users.length) {
    return (
      '<div class="table-card"><div class="sec-h"><div class="sec-t">' + title + '</div>' +
      '<div class="sec-c">0 ' + L.verifiedWord + '</div></div>' +
      '<div class="empty" id="empty_' + secId + '">' + L.noVerifiedTitle + '<div class="sub">' + L.noVerifiedSub + '</div></div></div>'
    );
  }
  const rows = users.map((u, i) => rowHtml(u, results, i)).join('\n');
  return (
    '<div class="table-card">' +
    '<div class="sec-h"><div class="sec-t">' + title + '</div><div class="sec-c">' + fmtInt(users.length) + ' ' + L.verifiedWord + '</div></div>' +
    '<div class="table-scroll"><table>' +
    '<thead><tr><th>#</th><th>' + L.thUser + '</th><th>' + L.thFollowers + '</th><th>' + L.thBio + '</th></tr></thead>' +
...
function buildHtmlReport(opts) {
  ...
  const L = REPORT_I18N[opts.lang === 'en' ? 'en' : 'ru'];
  const uniqueCount = new Set([...following, ...followers].map((u) => (u.handle || '').toLowerCase())).size;
  const generatedAt = new Date().toLocaleString('en-US', { dateStyle: 'medium', timeStyle: 'short' });

  const statsHtml =
    '<div class="stat"><div class="v">' + fmtInt(uniqueCount) + '</div><div class="k">' + L.verifiedTotal + '</div></div>' +
    '<div class="stat"><div class="v">' + fmtInt(following.length) + '</div><div class="k">' + L.following.toLowerCase() + '</div></div>' +
    '<div class="stat"><div class="v">' + fmtInt(followers.length) + '</div><div class="k">' + L.followers.toLowerCase() + '</div></div>';

  const sections =
    sectionHtml('following', L.following, following, results, L) + '\n' +
    sectionHtml('followers', L.followers, followers, results, L);
  ...
  const inlineScript = [
    ...
    '    setTimeout(function () { copy.textContent = "' + L.copy + '"; }, 1500);',
    '  });',
    '})();'
  ].join('\n');

  return '<!DOCTYPE html>\n' +
    '<html lang="' + (opts.lang === 'en' ? 'en' : 'ru') + '">\n<head>\n' +
    '<meta charset="utf-8">\n' +
    '<meta name="viewport" content="width=device-width, initial-scale=1">\n' +
    '<title>ScrXper — @' + handle + ' — ' + (opts.lang === 'en' ? 'report' : 'отчёт') + '</title>\n' +
    '<style>\n' +
    ':root {\n' +
    ...
    '<div class="wrap">\n' +
    '  <header class="card">\n' +
    '    <div class="brand"><span class="logo">📡</span><b>ScrXper</b> · ' + L.profileReport + '</div>\n' +
    '    <div class="badge">' + L.verifiedOnly + '</div>\n' +
    '    <h1><span class="at">@</span>' + escHtml(handle) + '</h1>\n' +
    '    <div class="sub">' + fmtInt(following.length) + ' ' + L.verifiedWord + ' ' + L.following.toLowerCase() + ' · ' + fmtInt(followers.length) + ' ' + L.verifiedWord + ' ' + L.followers.toLowerCase() + ' · <a href="https://x.com/' + escHtml(handle) + '" target="_blank" rel="noopener" style="color:#1d9bf0;text-decoration:none">x.com/' + escHtml(handle) + '</a></div>\n' +
    '    <div class="stats">' + statsHtml + '</div>\n' +
    '    <div class="toolbar">\n' +
    '      <input id="q" class="search" placeholder="' + L.searchPh + '" autocomplete="off" spellcheck="false">\n' +
    '      <button id="copy" class="copy" title="' + L.copyTitle + '" type="button">' + L.copy + '</button>\n' +
    '    </div>\n' +
    '  </header>\n' +
    sections + '\n' +
    '  <div class="table-card" id="emptyCard" hidden><div class="empty">' + L.noMatches + '<div class="sub">' + L.noMatchesSub + '</div></div></div>\n' +
    '  <footer>' + L.footer1.replace('{v}', version).replace('{date}', generatedAt) + '<br>' + L.footer2 + '</footer>\n' +
    '</div>\n' +
    '<script>\n' + inlineScript + '\n' +
    '</script>\n' +
    '</body>\n</html>\n';
